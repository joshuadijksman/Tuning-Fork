...
class E625:
    ...
    def relative_voltage(self, voltage) -> None:
        self.pidevice.SVO(self.target, False)
        self.pidevice.SVR(self.target, voltage)

    def absolute_voltage(self, voltage) -> None:
        self.pidevice.SVO(self.target, False)
    ...
    def thread_for_voltage(self, start, end, step, waittime) -> None:
        v = start
    ...
    def for_voltage(self, start, end, step, waittime) -> None:
        # Must protect self._stop with a mutex because a secondary thread
        # might try to access it at the same time.
