"""
Inspired from `rigol_dg1022` package:
...
class DS1054Z:
    """
    Module to control Rigol DSx Function Generators.
    Tested with DS1045Z
    Supports USB or LAN connections. (LAN untested)
    """
    ...
    def __init__(
        ...
        """
        Initialize the connection to the Rigol DSx.

        :param resource_string: VISA resource string. If None, attempts to find the device automatically. Selects first found.
        ...
        try:
            if DS1054Z._resource_manager is None:
                DS1054Z._resource_manager = pyvisa.ResourceManager()

            self.rm = DS1054Z._resource_manager

            if resource_string is None:
                if ip_address is not None:
                    resource_string = f"TCPIP0::{ip_address}::INSTR"
                    self.logger.info(
                        f"Using network resource string: {resource_string}"
                    )
                else:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                raise Exception(
                    f"Failed to connect to device after {max_retries} attempts"
                )

            # Verify device identity
            # Decrease timeout for this operation
            self.inst.timeout = 100
            idn = str(self._query("*IDN?"))
            self.inst.timeout = timeout
            if "Rigol Technologies,DS".upper() not in idn.upper():
                raise ValueError(f"Connected device is not a Rigol DS: {idn}")

            self.logger.info(f"Successfully connected to Rigol DS: {idn}")

        except Exception as e:
            self.logger.error(f"Failed to initialize Rigol DSx: {str(e)}")
            self.close()
            raise
    ...
    def _send_command(
        self, command, check_errors=False, delay=0.01, max_retries=3
    ) -> None:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                time.sleep(
                    delay
                )  # Introduce a delay to avoid overloading communication
                if check_errors:
                ...
                self.logger.warning(
                    f"Attempt {attempt + 1}: Communication error while sending command '{command}': {str(e)}"
                )
                ...
                self.logger.error(
                    f"Unexpected error while sending command '{command}': {str(e)}"
                )
                raise

    def _query(self, query, max_retries=3, retry_delay=0.5) -> str | None:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                response: str = self.inst.query(query).strip()
                self.logger.debug(f"Received response: {response}")
                return response
            except pyvisa.errors.VisaIOError as e:
                self.logger.warning(
                    f"Attempt {attempt + 1}: Communication error while querying '{query}': {str(e)}"
                )
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    self._reconnect()
                ...
                self.logger.error(
                    f"Unexpected error while querying '{query}': {str(e)}"
                )
                raise

    def _reconnect(self, max_retries=3) -> None:
        """
        ...
        current_resource: str | None = getattr(self, "current_resource", None)
        ...
        for attempt in range(max_retries):
            try:
                if current_resource:
                    ...
                    self.logger.info(
                        f"Successfully reconnected to resource: {current_resource}"
                    )
                    ...
                    try:
                        ...
                        if "Rigol Technologies,DS".upper() not in idn.upper():
                            raise ValueError(
                                f"Connected device is not a Rigol DSx: {idn}"
                            )
                ...
                for res in resources:
                    if (
                        ...
                        try:
                            ...
                            if "Rigol Technologies,DS".upper() in idn.upper():
                                self.logger.info(f"Found Rigol DSx at: {res}")
                ...
                self.logger.warning(f"Attempt {attempt + 1}: Could not find Rigol DSx")
                time.sleep(1)
        ...
        raise IOError("Failed to reconnect to Rigol DSx")

    def _find_device(self) -> str:
        """
        Attempt to find the Rigol DS device automatically.
        """
        resources = self.rm.list_resources()
        for res in resources:
            if "DS" in res:
                ...
                return res
        raise ValueError("Could not find Rigol DSx device")

    def _format_value(self, value, decimals) -> str:
        """
        ...
        self.logger.debug(
            f"Formatted value: {formatted} (Original: {value}, Decimals: {decimals})"
        )
        return formatted

    def _round_value(self, value, decimals) -> int | float:
        """
        ...
        self.logger.debug(
            f"Rounded value: {rounded} (Original: {value}, Decimals: {decimals})"
        )
        return rounded

    ###################################################################################################
    ###################################################################################################
    # TODO: Remove and add new functions
    ###################################################################################################
    ###################################################################################################
    ...
    def AUToscale(self) -> None:
        ...
        self.logger.info("Sent AUToscale")

    def CLEar(self) -> None:
        """
        Clear all waveforms on screen.
...
###################################################################################################
    ...
    def set_output(self, channel, state):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set output for channel {channel} to {'ON' if state else 'OFF'}"
        )
    ...
    def set_phase(self, channel, phase):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set phase for channel {channel} to {formatted_phase} degrees"
        )
    ...
    def set_impedance(self, channel=1, impedance=50):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set output impedance for channel {channel} to {impedance_cmd}"
        )
    ...
    def get_phase(self, channel):
        ...
        self.logger.info(
            f"Current phase for channel {channel}: {rounded_phase} degrees"
        )
    ...
    def get_output_state(self, channel):
        ...
        self.logger.info(
            f"Current output state for channel {channel}: {'ON' if state else 'OFF'}"
        )
        return state
    ...
    def set_burst_internal_period(self, channel, period):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set burst internal period to {period} seconds for channel {channel}"
        )
    ...
    def get_burst_internal_period(self, channel):
        ...
        self.logger.info(
            f"Burst internal period for channel {channel} is {period} seconds"
        )
        return period
    ...
    def set_burst_state(self, channel, state):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set burst state to {'ON' if state else 'OFF'} for channel {channel}"
        )
    ...
    def get_burst_state(self, channel):
        ...
        state = response.strip().upper() == "ON"
        self.logger.info(
            f"Burst state for channel {channel} is {'ON' if state else 'OFF'}"
        )
        return state
    ...
    def set_burst_gate_polarity(self, channel, polarity):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set burst gate polarity to {polarity.upper()} for channel {channel}"
        )
    ...
    def set_sweep_spacing(self, channel, spacing):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set sweep spacing to {spacing.upper()} for channel {channel}"
        )
    ...
    def set_sweep_state(self, channel, state):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set sweep state to {'ON' if state else 'OFF'} for channel {channel}"
        )
    ...
    def get_sweep_state(self, channel):
        ...
        state = response.strip().upper() == "ON"
        self.logger.info(
            f"Sweep state for channel {channel} is {'ON' if state else 'OFF'}"
        )
        return state
    ...
    def set_sweep_start_frequency(self, channel, frequency):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set sweep start frequency for channel {channel} to {formatted_freq} Hz"
        )
    ...
    def get_sweep_start_frequency(self, channel):
        ...
        rounded_freq = self._round_value(frequency, self.FREQ_DECIMALS)
        self.logger.info(
            f"Sweep start frequency for channel {channel} is {rounded_freq} Hz"
        )
        return rounded_freq
    ...
    def set_sweep_stop_frequency(self, channel, frequency):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set sweep stop frequency for channel {channel} to {formatted_freq} Hz"
        )
    ...
    def get_sweep_stop_frequency(self, channel):
        ...
        rounded_freq = self._round_value(frequency, self.FREQ_DECIMALS)
        self.logger.info(
            f"Sweep stop frequency for channel {channel} is {rounded_freq} Hz"
        )
        return rounded_freq
    ...
    def set_sweep_trigger_source(self, channel, source):
        ...
        self._send_command(command)
        self.logger.info(
            f"Set sweep trigger source to {source.upper()} for channel {channel}"
        )
    ...
    def apply_waveform(
        self, channel, waveform, frequency=None, amplitude=None, offset=None, phase=None
    ):
        """
        ...
        self.logger.info(
            f"Applied waveform '{waveform}' to channel {channel} with parameters: frequency={frequency}, amplitude={amplitude}, offset={offset}"
        )
